...
public class PerfStats
{
    ...
    public void DisplayStats(TextRenderer textRenderer, Camera camera, Scene scene, Scene? skyboxScene, float x = 0.02f, float y = 0.05f, float scale = 11f)
    {
        ...
        AddLine($"Render state:     {counts[(int)Counter.RenderStateApply]:N0} applies, {counts[(int)Counter.RenderStateGroupEmit]:N0} state groups emitted, {counts[(int)Counter.RenderStateGlCall]:N0} raw GL state calls this frame", valueColor);
