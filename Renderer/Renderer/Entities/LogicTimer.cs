...
/// The clock behind anything in a map that happens by itself.
/// </summary>
public sealed class LogicTimer : BaseEntity
{
    ...
    /// <summary>
    /// Initializes a <c>logic_timer</c> from its keyvalues.
    /// </summary>
    ...
    /// <summary>Starts the timer, scheduling its next firing a full interval away.</summary>
    ...
    /// <summary>Changes the interval. The firing already scheduled keeps its old time.</summary>
    /// <param name="data">Carries the new interval in seconds.</param>
    [EntityInput("RefireTime")]
    private void InputRefireTime(EntityInputData data) => RefireTime = data.Float(RefireTime);

    /// <summary>Restarts the interval from now.</summary>
    /// <param name="data">The input's parameter and sender, unused.</param>
    ...
    /// <summary>Fires <c>OnTimer</c> now, whatever the clock says, and restarts the interval.</summary>
    /// <param name="data">The input's parameter and sender, unused.</param>
