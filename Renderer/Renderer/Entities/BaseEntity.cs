...
public class BaseEntity
{
    ...
    /// <summary>
    /// Gets or sets whether the entity's geometry is drawn, Source's <c>EF_NODRAW</c>. Every node it owns
    /// follows, and a visibility layer being switched off still hides it either way.
    /// </summary>
    public bool IsDrawn
    {
        get;
        set
        {
            ...
            foreach (var node in ownedNodes)
            {
                Scene.MarkParentOctreeDirty(node);
            }
        }
    } = true;
    ...
    protected virtual SceneNode? CreateRootNode()
    {
        ...
        // Hidden with the other editor-only markers rather than with the world, as the loader draws it for
        // an unimplemented classname. Geometry an entity really has stays on the entity's own layer.
        return new SimpleBoxSceneNode(Scene, color, new Vector3(16f))
        {
            LayerName = World.WorldLoader.ToolEntitiesLayerName,
        };
    }
